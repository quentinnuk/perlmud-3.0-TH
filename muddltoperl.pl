# tidied up with perltidy -l=0 -fnl -noll -csc -csci=10 -cscw -msc=1
    ...
    while ( $id = <IN> ) {
        if ( !$dbVersionKnown ) {
            ...
            if ( !( $id =~ /^\d+\.\d+\s*$/ ) ) {
                ...
                next;
            } ## end else [ if ( !( $id =~ /^\d+\.\d+\s*$/...))]
        } ## end if ( !$dbVersionKnown )
        chomp $id;
        if ( $dbVersion >= 2.1 ) {
                ...
                if ( $nextarg =~ /^[-]?\d+$/ ) {            # message or demon or 0
                    ...
                    if ( ( $dest == 0 ) && ( $nextarg =~ /.*\|.*/ ) ) { # multi destination
                        ...
                        foreach my $direction (@travelargs) { # expand directions inc. random options
                            ...
                            print LOG "$direction\n";
                        } ## end foreach my $direction (@travelargs)
...
sub do_texts { # stores all the texts reponses into a list for lookup later
    ...
    for ( $i = 0 ; $i <= $#objects ; $i++ ) { # could use a foreach here but having the index aids debugging
        if ( $objects[$i]{"type"} == $exit ) {
            ...
            if ( $objects[$i]{"action"} =~ /mud_send(effect|mess)/i ) { # special handler for messages in sendeffect and sendmess
                $objects[$i]{"action"} =~ /^(.*\'\,\')(\d+)(\'\,.*$)/i; # look for the message id surrounded by quotes
                my $msg = $textIds{$2};                                 # resolves message id
                $objects[$i]{"action"} = $1 . $msg . $3;
            }
        } ## end elsif ( $objects[$i]{"type"...})
    } ## end for ( $i = 0 ; $i <= $#objects...)
    print LOG "end texts\n";
    print "Done\n";
} ## end sub do_texts
...
sub do_vocab
{
    ...
    while ( $line = read_line() ) {
        ...
        } ## end elsif ( $subsection eq "action")
